NB.BoardViewModel = function (): void {
    ...
    self.filterBoard = function (): void {
        self.forEachNoteVM(function (note) {
            ...
            if (self.curActiveCategory.name() === self.generalCategoryName || self.curActiveCategory.name() === note.category()) {
                sameCategory = true;
            }
    ...
    self.clickSave = function (): void {
        self.forEachNoteVM(function (note) {
            if (note.canDiscard()) {
                NB.board.updateNoteById(note.id, note.content(), Date.now(), note.category());
    ...
    self.discardNote = function (note): void {
        ...
        oldNote ? self.NotesVM.replace(note, new NB.NoteVM(oldNote)) : self.removeNote(note);
    };
...
NB.BoardViewModel.prototype = {
    ...
    getCategoryName: function () {
        ...
        const defaultNames = categories.filter((category) => category.name.includes("default")).sort((cat1, cat2) => {
            return cat1.name - cat2.name
        });
        const name = defaultNames.length ? defaultNames[defaultNames.length - 1].name : 'default';
    ...
    categoryMoveUp: function (categories, event) {
        ...
        this.categoriesVM.replace(this.categoriesVM()[index - 1], this.curActiveCategory);
